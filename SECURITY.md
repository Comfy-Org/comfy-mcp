...
- **Read and write files as the user.** `run_workflow(workflow_path)` and `validate_workflow(workflow_path)` read a path you name. Over stdio, `upload_file(file_path, client_os)` reads that absolute path; over HTTP it returns a single-use PUT command, and only bytes sent to that capability URL are copied into the target ComfyUI's `input` directory. That target is on **another machine** when the operator configured `COMFYUI_URL`/`COMFYUI_HOST`, so uploaded bytes leave this host over that network (the destination is operator environment config, never a tool input, so a compromised agent cannot choose it). Over stdio, `fetch_outputs(out_dir)`, `fetch_template(out_path)`, and `emit_partner_workflow(out_path)` write to a path you name. Over HTTP, `fetch_outputs` instead keeps downloaded files in temporary server storage and returns a five-minute signed bearer URL on the MCP listener; anyone holding that URL can read the file until it expires. The server does not sandbox the filesystem.
...
- **Argument and flag injection into comfy-cli.** Tool-facing positionals are bounded and NUL bytes are rejected wherever they ride (`_reject_nul`); `upload_file.file_path` must be absolute and is passed after the fixed `upload` verb, job/download handles are format-bounded (`_guard_prompt_id`, `_guard_download_id`), and `update_comfyui`'s `target` is validated against an allowlist so only the matched value — never the caller's raw string — reaches the command line.
...
- **Remote file-transfer containment.** Upload URLs bind an unguessable, single-use token to one validated image filename, expire after five minutes, and accept at most 50 MiB. The route writes owner-only scratch, passes only that generated path to comfy-cli, never records request bytes in the failure log, and removes scratch on every exit. Signed output URLs bind an unguessable token, filename, and expiry with HMAC; only regular files that comfy-cli reported inside the dedicated download scratch root can be published. Responses are non-cacheable and scratch is removed at expiry. Treat complete capability URLs as temporary secrets and configure a reverse proxy to protect `/mcp`, `/api/uploads/`, and `/downloads/`.
- **Resource exhaustion of the server itself.** Timeouts are bounded with hard ceilings (`_bounded_timeout`), output drains are capped to a trailing window (`_drain_capped_async`), a timeout kills the whole process tree rather than orphaning a `git pull` or a multi-GB transfer (`_kill_proc_tree`), capability uploads are capped at 50 MiB, and inline image previews are capped at 8 files / 16 MiB aggregate so a large batch cannot blow up a request or reply.
