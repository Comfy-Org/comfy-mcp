...
- **Read and write files as the user.** `run_workflow(workflow_path)` and `validate_workflow(workflow_path)` read a path you name; `upload_file(paths)` copies any user-readable server path—or bytes an HTTP client deliberately supplied inline—into the target ComfyUI's `input` directory. That target is on **another machine** when the operator configured `COMFYUI_URL`/`COMFYUI_HOST`, so those bytes leave this host over that network (the destination is operator environment config, never a tool input, so a compromised agent cannot choose it). Over stdio, `fetch_outputs(out_dir)`, `fetch_template(out_path)`, and `emit_partner_workflow(out_path)` write to a path you name. Over HTTP, `fetch_outputs` instead keeps downloaded files in temporary server storage and returns a 10-minute signed bearer URL on the MCP listener; anyone holding that URL can read the file until it expires. The server does not sandbox the filesystem.
...
- **Remote file-transfer containment.** Inline upload names are basenames only, strict base64 is capped at 2 MiB decoded per call, scratch directories/files are owner-only, and inline bytes never enter subprocess argv or the failure log. Signed output URLs bind an unguessable token, filename, and expiry with HMAC; only regular files that comfy-cli reported inside the dedicated download scratch root can be published. Responses are non-cacheable and scratch is removed at expiry. Treat the complete URL as a temporary secret and configure a reverse proxy to protect both `/mcp` and `/downloads/`.
- **Resource exhaustion of the server itself.** Timeouts are bounded with hard ceilings (`_bounded_timeout`), output drains are capped to a trailing window (`_drain_capped_async`), a timeout kills the whole process tree rather than orphaning a `git pull` or a multi-GB transfer (`_kill_proc_tree`), inline uploads are capped below the SDK's request-body limit, and inline image previews are capped at 8 files / 16 MiB aggregate so a large batch cannot blow up a request or reply.
