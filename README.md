...
- **Discovery / validation** (`nodes`, `validate_workflow`, and the
  ...
  describe the **local** install by default. `nodes` and `validate_workflow` can instead read a
  saved catalog through `object_info_path`; this is an offline snapshot override, not a remote
  fetch. The template `local_check` remains local-only. A workflow or template can still pass a
  local check and fail on a remote whose node set differs, so validate against a local ComfyUI or
  saved catalog matching the remote's.
...
| `generate_image(prompt, checkpoint=None, wait=True, timeout_seconds=60.0)` | `comfy run-template image_z_image_turbo --param=57.text=<prompt>` | Text prompt → image in one call, with no hand-assembled workflow needed: it runs the built-in text-to-image gallery template (`image_z_image_turbo`) through the same verb (and the same run path) as `run_template`. `checkpoint` needs a graph with a checkpoint slot, and the built-in one has none, so passing it is refused by name until `COMFY_T2I_CHECKPOINT_SLOT` points at a template that has one. Free — nothing here spends credits. Runs on whichever ComfyUI the server targets, so it follows `COMFYUI_URL`/`COMFYUI_HOST` like `run_workflow` does ([Driving a remote ComfyUI](#driving-a-remote-comfyui)); a checkpoint, where the template takes one, has to be installed on *that* machine. Retarget the template with [`COMFY_T2I_TEMPLATE` and its slot-key companions](#prerequisites). Same envelope shape as `run_workflow` (`prompt_id` + outputs); the fast on-ramp. `timeout_seconds` bounds this server's wait only — your MCP client's own transport cap applies too, and the shorter one wins, so the default is small enough to expire on this side first. An expired wait is not an error: it returns `{"timed_out": true, "prompt_id": …, "status": …, "note": …}` for a generation that is still running, so the job stays pollable with `job(action="status")`, collectable with `fetch_outputs` and cancellable with `job(action="cancel")` instead of being orphaned. On slow hardware prefer `wait=False` and poll. |
...
| `billing_status()` | `comfy cloud status` | Comfy Cloud credit balance, subscription tier and job concurrency for the signed-in workspace — **read-only, spends nothing**, so it raises no spend confirmation. Answers "how many credits do I have", "what plan am I on", "when does it renew", "how many jobs can I run at once". Returns comfy-cli's payload as-is: `cloud_workspace`, `credit_balance_usd` / `credit_balance_credits` / `effective_balance_micros`, `currency`, `balance_confirmed`, `subscription` (tier / status / is_active / plan_slug / renewal_date / cancel_at), `max_concurrent_jobs`, `upgrade_suggestion`, `manage_url` and `message`. `balance_confirmed: false` means the balance fields are `null` because none could be *established* — not that the balance is zero — and `message` is comfy-cli's own neutral copy for that case, to be relayed verbatim rather than re-derived from the numbers. Signed out raises comfy-cli's own error (`auth_login` / `auth_status` are the fix); on a comfy-cli predating the verb — or predating the whole `comfy cloud` group — it degrades to `{"error": …, "unsupported": true, "balance_confirmed": false}` — with the three balance fields carried as `null` — naming the upgrade, the same benign capability gap as `server_info`'s `freshness` block (the verdict and the fields it is about travel with the degrade, so the `balance_confirmed`-then-read rule holds on that shape too). |
...
| Tool | Wraps | What it does |
|---|---|---|
| `validate_workflow(workflow_path, object_info_path="")` | `comfy validate --workflow <path> [--input <object_info.json>]` | Pre-flight a workflow against the live `object_info` before a slow run, or pass `object_info_path` to validate against a saved catalog when the live response is too large. Returns comfy-cli's own report — `{"valid": bool, "error_count", "errors": [...], "warnings": [...]}` — where each error names the `node_id` (subgraph-qualified as `105:11`), the `field`, a machine `code`, and often `suggestions` / `valid_options` naming what your install actually has (all optional keys — read them with `.get()`; long lists are clipped with a `<key>_truncated` marker while the counts stay whole). **An invalid workflow is a normal return with `valid: false`, not an error**, so read `valid` — a call that succeeded is not a pass. An exception means no verdict came back: usually the check could not run (ComfyUI isn't up and no saved catalog was supplied), sometimes comfy-cli failed the command outright — never a pass, and not a per-node verdict. |
...
read the **user's live install** (custom nodes included) by default, not a static catalog —
that's the local differentiator from the cloud MCP's equivalents. `nodes` can instead read
a saved `object_info` snapshot through `object_info_path`. The graph-wiring actions (`"upstream"` /
