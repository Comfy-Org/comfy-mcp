...
> **Status:** beta. 48 tools; core loop validated end-to-end against a live local ComfyUI
...
48 tools, grouped below by what they do. Every tool runs `comfy` with the global
...
| `wait_for_download(download_id, timeout_seconds=25.0)` | `comfy model download-status <download_id>` (polled) | Bounded wait until a download reaches a terminal state (completed / failed / cancelled); returns a `{"timed_out": True, …}` payload on expiry. Chain several — the `job(action="wait")` shape, for transfers. Degrades to `unsupported: true` on a comfy-cli without the verb. |
