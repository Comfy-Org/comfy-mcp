...
Nine **leaf** modules sit under it — none imports `server`, so the dependency
...
| `params.py` | param/slot marshaling into comfy-cli argv for `comfy generate` / `run-template` / `workflow set-slot` / `workflow vary`: `_generate_param_args`, `_run_template_param_args`, `_validate_param_key`, `_validate_generate_model` (+ `_GENERATE_RESERVED_TARGETS`, `_GENERATE_META_FLAGS`), and the structured slot machinery — `SlotOverride`, `SlotVariants`, `_slot_address_arg`, `_slot_value_json`, `_as_slot_model`, `_slot_override_arg`, `_slot_variants_arg`, `_reject_non_json_array_slot`. `SlotOverride`/`SlotVariants` are this module's public TYPES (see the carve-out below) — they ride `@mcp.tool()` signatures, so `server` name-imports them to keep the exported MCP schema's `$defs` identical |
