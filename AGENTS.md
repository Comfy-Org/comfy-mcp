...
(foreground `model download`, `workflow_deps`, `upload_file`, and `job("watch")`'s status
polls). Each stream keeps only a `_STDERR_MAX_CHARS` tail (`_drain_capped_into`; callers
widen stdout via `stdout_cap=`), never `communicate()`'s full capture. `auth_login`
(`_start_login`) is a third spawn site.
