...
carries the legacy foreground `model download`, `workflow_deps`'/`upload_file`'s 300s calls,
and `job("watch")`'s status polls. Each stream keeps only a `_STDERR_MAX_CHARS` tail
