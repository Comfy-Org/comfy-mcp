...
CHECK whether a free twin exists — `nodes(action="search")` and
`search_templates` WITHOUT `exclude_api`, reading those markers — and when
both exist, present BOTH and let the USER choose: the paid path bills per run
via `confirm_spend`, the free path runs locally subject to the
hardware-routing steps above. Never assert the paid path is the only one
without having looked. When the request itself implies free / local / open
weights, pass `exclude_api=True` to `search_templates` — that filter always
exists; `nodes` takes no such argument, so screen its rows on the markers
yourself. Twin parameters do NOT carry across: a paid node's fixed
`resolution` combo does not bound the free one, which may take free-form
`width`/`height` — read the free node's own schema with
`nodes(action="get")` before telling a user a resolution is impossible. This
